...
if __name__ == '__main__':
    ...
    if not path.isfile(path.realpath(settings_file)):
        raise IOError("ERROR: to execute this script as standalone,"
    ...
    for md in search.get("results"):
        ...
        if not md_attributes:
            vectors_without_attributes.append("{} ({})".format(md.get("title"),
                                                               md.get("_id")))
            continue
        for field in md_attributes:
            ...
            attributes[field.get("name")] = (field.get("dataType"),
                                             field.get("alias", "NR"),
                                             field.get("description", "NR"))

    # global metrics
    print("{} attributes among "
          "{} metadatas retrieved of which "
          "{} do not have feature attributes."
          .format(len(attributes_names),
