...
class TestMailchimp(unittest.TestCase):
    ...
    def test_mailchimp_status_prod(self):
        ...
        client = MailChimp(mc_api=isogeo_mailchimp_api_prod,
                           mc_user="Unittest - Python SDK",
                           request_headers=self.headers,
                           timeout=10.0)
        p = client.ping
        self.assertDictEqual(
            p.get(), {'health_status': "Everything's Chimpy!"})

    def test_mailchimp_status_qa(self):
        """Check the mailchimp authentication and QA status."""
        client = MailChimp(mc_api=isogeo_mailchimp_api_qa,
                           mc_user="Unittest - Python SDK",
                           request_headers=self.headers,
                           timeout=30.0)
        p = client.ping
    ...
    def test_mailchimp_lists_qa(self):
        ...
        client = MailChimp(mc_api=isogeo_mailchimp_api_prod,
                           ...
                           request_headers=self.headers,
                           timeout=30.0)
        lists_subscribers = client.lists.all(get_all=True,
    ...
    def test_mailchimp_bad_api_key(self):
        ...
        with self.assertRaises(ValueError):
            client = MailChimp(mc_api="ImNotAValidApiKey",
                               mc_user="Unittest - Python SDK",
                               request_headers=self.headers,
                               timeout=10.0)
