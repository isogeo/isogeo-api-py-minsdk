...
class TestRoutes(unittest.TestCase):
    ...
    def test_resources_search(self):
        ...
        search = self.isogeo.search(self.bearer, page_size=1, whole_share=0, check=0)

        self.isogeo.resource(
            self.bearer, id_resource=search.get("results")[0].get("_id")
        )
    ...
    def test_thesauri(self):
        ...
        self.isogeo.thesaurus(self.bearer, thez_id=thesauri[0].get("_id"))

    def test_licenses(self):
        """Try to get licenses details."""
        search = self.isogeo.search(self.bearer, page_size=0, whole_share=0, check=0)
        ...
        licenses = self.isogeo.licenses(self.bearer, owner_id=wg)
        self.assertIsInstance(licenses, tuple)
        self.assertIn(401, licenses)

        # a specific license
        license = self.isogeo.license(
            self.bearer, license_id="f6e0c665905a4feab1e9c1d6359a225f"
        )
    ...
    def test_keywords(self):
        ...
        thez_id = self.isogeo.thesauri(self.bearer)[0].get("_id")
        # list tags
        thez_keywords = self.isogeo.keywords(self.bearer, thez_id=thez_id, page_size=1)
