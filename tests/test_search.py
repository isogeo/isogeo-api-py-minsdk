...
class TestSearch(unittest.TestCase):
    ...
    def test_search_length(self):
        ...
        # requests
        search_default = self.isogeo.search(
            self.bearer, whole_share=0  # default value= 100
        )
        search_10 = self.isogeo.search(self.bearer, page_size=10, whole_share=0)
        search_20 = self.isogeo.search(self.bearer, page_size=20, whole_share=0)
        search_50 = self.isogeo.search(self.bearer, page_size=50, whole_share=0)
        search_100 = self.isogeo.search(self.bearer, page_size=100, whole_share=0)
        search_sup100 = self.isogeo.search(self.bearer, page_size=103, whole_share=0)
        search_whole = self.isogeo.search(self.bearer, page_size=50, whole_share=1)
        search_rand = self.isogeo.search(self.bearer, page_size=rand, whole_share=0)
    ...
    def test_search_specific_mds_ok(self):
        ...
        # get random metadata within a small search
        search_ids_1 = self.isogeo.search(self.bearer, specific_md=[md_a])
        search_ids_2 = self.isogeo.search(self.bearer, specific_md=[md_a, md_b])
        search_ids_3 = self.isogeo.search(self.bearer, specific_md=[md_a, md_b, md_bad])
        # test length
    ...
    def test_search_specific_mds_bad(self):
        ...
        with self.assertRaises(TypeError):
            self.isogeo.search(self.bearer, page_size=0, whole_share=0, specific_md=md)

    # includes
    def test_search_includes_ok(self):
        """Searches including includes."""
        self.isogeo.search(
            self.bearer, page_size=0, whole_share=0, include=["links", "contacts"]
        )

    def test_search_includes_all_ok(self):
        """Searches including includes."""
        self.isogeo.search(self.bearer, page_size=0, whole_share=0, include="all")

    def test_search_includes_empty(self):
        """Search with empty includes list."""
        self.isogeo.search(self.bearer, page_size=0, whole_share=0, include=[])

    def test_search_includes_bad(self):
        """Include sub_resrouces require a list."""
        with self.assertRaises(TypeError):
            self.isogeo.search(self.bearer, page_size=0, whole_share=0, include="links")
    ...
    def test_search_parameter_query_ok(self):
        ...
        # format
        self.isogeo.search(self.bearer, query="format:shp", page_size=0, whole_share=0)
        # has-no
        self.isogeo.search(
            self.bearer, query="has-no:keyword", page_size=0, whole_share=0
        )
        # inspire themes
        self.isogeo.search(
            self.bearer,
        ...
        # keyword
        self.isogeo.search(
            self.bearer, query="keyword:isogeo:2018", page_size=0, whole_share=0
        )
        # licenses
        self.isogeo.search(
            self.bearer,
        ...
        # SRS
        self.isogeo.search(
            self.bearer, query="coordinate-system:2154", page_size=0, whole_share=0
        )
        # types
        self.isogeo.search(
            self.bearer, query="type:dataset", page_size=0, whole_share=0
        )
        self.isogeo.search(
            self.bearer, query="type:vector-dataset", page_size=0, whole_share=0
        )
        self.isogeo.search(
            self.bearer, query="type:raster-dataset", page_size=0, whole_share=0
        )
        self.isogeo.search(
            self.bearer, query="type:service", page_size=0, whole_share=0
        )
        self.isogeo.search(
            self.bearer, query="type:resource", page_size=0, whole_share=0
        )
        # workgroup - owner
        self.isogeo.search(
            self.bearer,
        ...
        # unknown
        self.isogeo.search(
            self.bearer, query="unknown:filter", page_size=0, whole_share=0
        )

    def test_search_bad_parameter_query(self):
        """Search with bad parameter."""
        with self.assertRaises(ValueError):
            self.isogeo.search(self.bearer, query="type:youpi")
        with self.assertRaises(ValueError):
            self.isogeo.search(self.bearer, query="action:yipiyo")
        with self.assertRaises(ValueError):
            self.isogeo.search(self.bearer, query="provider:youplaboum")
    ...
    def test_search_bad_parameter_geographic(self):
        ...
        with self.assertRaises(ValueError):
            ...
            self.isogeo.search(self.bearer, georel="intersects")
        with self.assertRaises(ValueError):
            # georel bad value
            self.isogeo.search(
                self.bearer, bbox="-4.970,30.69418,8.258,51.237", georel="cross"
            )
    ...
    def test_parameter_not_unique_search(self):
        ...
        with self.assertRaises(ValueError):
            self.isogeo.search(
                self.bearer, query="coordinate-system:32517 coordinate-system:4326"
            )
        with self.assertRaises(ValueError):
            self.isogeo.search(self.bearer, query="format:shp format:dwg")
        with self.assertRaises(ValueError):
            self.isogeo.search(
                self.bearer,
        ...
        with self.assertRaises(ValueError):
            self.isogeo.search(
                self.bearer, query="type:vector-dataset type:raster-dataset"
            )
        # disabling check, it should not raise anything
        self.isogeo.search(
            self.bearer, query="coordinate-system:32517 coordinate-system:4326", check=0
        )
        self.isogeo.search(self.bearer, query="format:shp format:dwg", check=0)
        self.isogeo.search(
            self.bearer,
            query="owner:32f7e95ec4e94ca3bc1afda960003882 owner:08b3054757544463abd06f3ab51ee491",
            check=0,
        ...
        self.isogeo.search(
            self.bearer, query="type:vector-dataset type:raster-dataset", check=0
        )
    ...
    def test_search_augmented(self):
        ...
        self.assertFalse(
            hasattr(self.isogeo, "shares_id")
        )  # at start, shares_id attribute doesn't exist
        # normal
        search = self.isogeo.search(self.bearer, page_size=0, whole_share=0, augment=0)
        ...
        # augmented
        search = self.isogeo.search(self.bearer, page_size=0, whole_share=0, augment=1)
        tags_shares = [i for i in search.get("tags") if i.startswith("share:")]
    ...
    def test_search_tags_dictionarized(self):
        ...
        search = self.isogeo.search(
            self.bearer, page_size=0, whole_share=0, check=0, tags_as_dicts=0
        )
        tags_normal = search.get("tags")
        # tags as dicts
        search = self.isogeo.search(
            self.bearer, page_size=0, whole_share=0, augment=1, tags_as_dicts=1
        )
        ...
        for k, v in sorted(tags_normal.items()):
            if k.startswith("action"):
                ...
                v = k.split(":")[
                    1
                ]  # handle specific case of tags which are only a bool value
    ...
    def test_get_formats(self):
        ...
        bad_specific_format = self.isogeo.get_formats(self.bearer, format_code=["shp"])
    ...
    def test_get_srs(self):
        ...
        # a specific srs
        wgs84 = self.isogeo.get_coordinate_systems(self.bearer, srs_code="4326")
        self.assertEqual(len(wgs84), 3)
        # if specific srs is bad formatted, so show all srs
        bad_specific_srs = self.isogeo.get_coordinate_systems(
            self.bearer, srs_code=["4326"]
        )
