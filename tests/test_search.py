...
class TestSearch(unittest.TestCase):
    ...
    def test_search_length(self):
        ...
        # requests
        search_default = self.isogeo.search(whole_share=0)  # default value= 100
        search_10 = self.isogeo.search(page_size=10, whole_share=0)
        search_20 = self.isogeo.search(page_size=20, whole_share=0)
        search_50 = self.isogeo.search(page_size=50, whole_share=0)
        search_100 = self.isogeo.search(page_size=100, whole_share=0)
        search_sup100 = self.isogeo.search(page_size=103, whole_share=0)
        search_whole = self.isogeo.search(page_size=50, whole_share=1)
        search_rand = self.isogeo.search(page_size=rand, whole_share=0)
    ...
    def test_search_specific_mds_ok(self):
        ...
        # get random metadata within a small search
        search_ids_1 = self.isogeo.search(specific_md=[md_a])
        search_ids_2 = self.isogeo.search(specific_md=[md_a, md_b])
        search_ids_3 = self.isogeo.search(specific_md=[md_a, md_b, md_bad])
        # test length
    ...
    def test_search_specific_mds_bad(self):
        ...
        with self.assertRaises(TypeError):
            self.isogeo.search(page_size=0, whole_share=0, specific_md=md)

    # includes
    def test_search_includes_ok(self):
        """Searches including includes."""
        self.isogeo.search(page_size=0, whole_share=0, include=["links", "contacts"])

    def test_search_includes_all_ok(self):
        """Searches including includes."""
        self.isogeo.search(page_size=0, whole_share=0, include="all")

    def test_search_includes_empty(self):
        """Search with empty includes list."""
        self.isogeo.search(page_size=0, whole_share=0, include=[])

    def test_search_includes_bad(self):
        """Include sub_resrouces require a list."""
        with self.assertRaises(TypeError):
            self.isogeo.search(page_size=0, whole_share=0, include="links")
    ...
    def test_search_parameter_query_ok(self):
        ...
        # catalog
        self.isogeo.search(
            query="catalog:633216a375ab48ca8ca72e4a1af7a266", page_size=0, whole_share=0
        )
        # CSW data-source
        self.isogeo.search(
        ...
        # format
        self.isogeo.search(query="format:shp", page_size=0, whole_share=0)
        # has-no
        self.isogeo.search(query="has-no:keyword", page_size=0, whole_share=0)
        # inspire themes
        self.isogeo.search(
        ...
        # keyword
        self.isogeo.search(query="keyword:isogeo:2018", page_size=0, whole_share=0)
        # licenses
        self.isogeo.search(
        ...
        # SRS
        self.isogeo.search(query="coordinate-system:2154", page_size=0, whole_share=0)
        # types
        self.isogeo.search(query="type:dataset", page_size=0, whole_share=0)
        self.isogeo.search(query="type:vector-dataset", page_size=0, whole_share=0)
        self.isogeo.search(query="type:raster-dataset", page_size=0, whole_share=0)
        self.isogeo.search(query="type:service", page_size=0, whole_share=0)
        self.isogeo.search(query="type:resource", page_size=0, whole_share=0)
        # workgroup - owner
        self.isogeo.search(
            query="owner:32f7e95ec4e94ca3bc1afda960003882", page_size=0, whole_share=0
        )
        # unknown
        self.isogeo.search(query="unknown:filter", page_size=0, whole_share=0)
    ...
    def test_search_augmented(self):
        ...
        # normal
        search = self.isogeo.search(page_size=0, whole_share=0, augment=0)
        ...
        # augmented
        search = self.isogeo.search(page_size=0, whole_share=0, augment=1)
        tags_shares = [i for i in search.get("tags") if i.startswith("share:")]
    ...
    def test_search_tags_dictionarized(self):
        ...
        search = self.isogeo.search(
            page_size=0, whole_share=0, check=0, tags_as_dicts=0
        )
        tags_normal = search.get("tags")
        # tags as dicts
        search = self.isogeo.search(
            page_size=0, whole_share=0, augment=1, tags_as_dicts=1
        )
    ...
    def test_app_properties(self):
        ...
        # add it
        self.isogeo.get_app_properties()
        # now it exists
    ...
    def test_get_srs(self):
        ...
        # all Isoego srs
        srs = self.isogeo.get_coordinate_systems()
        self.assertEqual(len(srs), 4301)
        self.assertEqual(len(srs[0]), 3)
        # a specific srs
        wgs84 = self.isogeo.get_coordinate_systems(srs_code="4326")
        self.assertEqual(len(wgs84), 3)
        # if specific srs is bad formatted, so show all srs
        bad_specific_srs = self.isogeo.get_coordinate_systems(srs_code=["4326"])
